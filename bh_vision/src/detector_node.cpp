...
#include "bh_vision/detector_node.hpp"
#include "bh_vision/detector_network.hpp"
#include "bh_vision/parse.hpp"
#include "bh_vision_msgs/msg/vision.hpp"
...
public:
  ...
  {
    bh_vision_msgs::msg::Vision::UniquePtr detections(
      new bh_vision_msgs::msg::Vision);
  ...
  rclcpp::Publisher<bh_vision_msgs::msg::Vision>::SharedPtr detections_pub_;
...
: rclcpp::Node("detector_node", options), impl_(new DetectorNodePrivate)
{
  ...
  const std::string network_config_path = declare_parameter<std::string>(
    network_cfg_desc.name, "install/bh_vision/share/bh_vision/config/yolo-v7-tiny/yolov7-tiny.cfg");
  ...
  const std::string network_weights_path = declare_parameter<std::string>(
    network_weights_desc.name, "install/bh_vision/share/bh_vision/config/yolo-v7-tiny/yolov7-tiny.weights");
  ...
  const std::string network_class_names_path = declare_parameter<std::string>(
    network_class_names_desc.name, "install/bh_vision/share/bh_vision/config/yolo-v7-tiny/coco.names");
  ...
  impl_->detections_pub_ = this->create_publisher<bh_vision_msgs::msg::Vision>(
    "~/detections", 1);
...
RCLCPP_COMPONENTS_REGISTER_NODE(bh_vision::darknet_ros::DetectorNode)
