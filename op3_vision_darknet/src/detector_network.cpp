...
#include "op3_vision/detector_network.hpp"

namespace op3_vision
{
...
: impl_(new DetectorNetworkPrivate())
{
  ...
  const int clear = 0;
  impl_->network_ = load_network_custom(&*config_mutable, &*weights_mutable, 0, 1);
...
DetectorNetwork::detect(
  ...
  op3_vision_msgs::msg::Vision * output_detections)
{
  ...
  for (int i = 0; i < num_detections; ++i) {
    ...
    float img_x_center = image_msg.width / 2.0f;
    float img_y_center = image_msg.height / 2.0f;

    float delta_x = detection.bbox.x - img_x_center;
    float delta_y = detection.bbox.y - img_y_center;
    // Get aspect ratio
    const auto aspect_ratio = static_cast<float>(image_msg.width / image_msg.height);

    // convert to radians and calculate the vertical and horizontal FOVs
    // respectively
    const auto diagonal_fov_rad = 78.0f * M_PI / 180.0f;

    float fov_horiz = 2 * std::atan(std::tan(diagonal_fov_rad / 2) / std::sqrt(1 + aspect_ratio * aspect_ratio));
    float fov_vert = 2 * std::atan(std::tan(diagonal_fov_rad / 2) / std::sqrt(1 + 1 / (aspect_ratio * aspect_ratio)));

    float angle_per_pixel_horiz = fov_horiz / image_msg.width;
    float angle_per_pixel_vert = fov_vert / image_msg.height;

    // Calculate bearing
    detection_ros.bearing.x = -1*(delta_x * angle_per_pixel_horiz);
    detection_ros.bearing.y = -1*(delta_y * angle_per_pixel_vert);
