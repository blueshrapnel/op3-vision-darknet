...
from bh_vision_msgs.msg import Vision
...
class DetectionVisualizerNode(Node):
    ...
    def __init__(self):
        ...
        self._image_sub = self.create_subscription(Image, '/camera/image_raw', self.on_image, qos_profile_sensor_data)
        self._detections_sub = self.create_subscription(Vision, '/camera/detections', self.on_detections, 1)
    ...
    def on_detections(self, detections_msg):
        # ensure image_message is defined
        if not hasattr(self, 'image_message'):
            self.get_logger().warn("Received detection before image.  Waiting.")
            return

        cv_image = self._bridge.imgmsg_to_cv2(self.image_message)
        ...
        detection_image_msg = self._bridge.cv2_to_imgmsg(cv_image, encoding=self.image_message.encoding)
        detection_image_msg.header = self.image_message.header
